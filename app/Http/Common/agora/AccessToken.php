...
//non reversable values
    ...
    function build()
    {
        ...
        $content = array_merge(unpack("C*", $this->packStrings($sig)), unpack("C*", pack("V", $crc_channel_name)), unpack("C*", pack("V", $crc_uid)), unpack("C*", pack("v", count($msg))), $msg);
        $version = "006";
        $ret = $version . $this->appID . base64_encode(implode(array_map("chr", $content)));
        return $ret;
    }
    function packStrings($value)
    {
